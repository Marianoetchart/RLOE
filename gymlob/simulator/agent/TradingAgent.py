...
from gymlob.simulator.util.order.MarketOrder import MarketOrder
from gymlob.simulator.util.util import log_print, fmtTime
...
class TradingAgent(FinancialAgent):
    ...
    def __init__(self, id, name, type, random_state=None, starting_cash=100000, log_orders=False, log_to_file=True):
        ...
        # agents must request pricing when it wants it.  This agents does NOT
        ...
        # When a last trade price comes in after market close, the trading agents
        ...
        # The agents remembers the last known bids and asks (with variable depth,
        # showing only aggregate volume at each price level) when it receives
        # a response to QUERY_SPREAD.
        self.known_bids = {}
        self.known_asks = {}

        # The agents remembers the order history communicated by the exchange
        # when such is requested by an agents (for example, a heuristic belief
        # learning agents).
        self.stream_history = {}

        # The agents records the total transacted volume in the exchange for a given symbol and lookback period
        self.transacted_volume = {}

        # Each agents can choose to log the orders executed
        self.executed_orders = []

        # For special logging at the first moment the simulator kernel begins
        # running (which is well after agents init), it is useful to keep a simple
        # boolean flag.
    ...
    def kernelStarting(self, startTime):
        ...
        log_print("Agent {} requested agents of type Agent.ExchangeAgent.  Given Agent ID: {}",
    ...
    def wakeup(self, currentTime):
        ...
        # indicating whether the agents is "ready to trade" -- has it received
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body['msg'] == "WHEN_MKT_OPEN":
            ...
            log_print("Recorded market open: {}", fmtTime(self.mkt_open))

        elif msg.body['msg'] == "WHEN_MKT_CLOSE":
            self.mkt_close = msg.body['data']

            log_print("Recorded market close: {}", fmtTime(self.mkt_close))
        ...
        if have_mkt_hours and not had_mkt_hours:
            ...
            # this as a subclass request so each agents can supply an appropriate offset relative
    ...
    def get_transacted_volume(self, symbol, lookback_period='10min'):
        """ Used by any trading agents subclass to query the total transacted volume in a given lookback period """
    ...
    appear in the agents's open orders list."""
    ...
    def modifyOrder(self, order, newOrder):
        ...
        appear in the agents's open orders list.  Some additional tests might be useful here
    ...
    # Handles ORDER_EXECUTED messages from an exchange agents.  Subclasses may wish to extend,
    ...
    def orderExecuted(self, order):
        ...
        log_print("After execution, agents open orders: {}", self.orders)

        # After execution, log holdings.
        self.logEvent('HOLDINGS_UPDATED', self.holdings)

    # Handles ORDER_ACCEPTED messages from an exchange agents.  Subclasses may wish to extend.
    def orderAccepted(self, order):
        ...
        # We may later wish to add a status to the open orders so an agents can tell whether
        # a given order has been accepted or not (instead of needing to override this method).

    # Handles ORDER_CANCELLED messages from an exchange agents.  Subclasses may wish to extend.
    ...
    # Handles MKT_CLOSED messages from an exchange agents.  Subclasses may wish to extend.
    ...
    # Handles QUERY_LAST_TRADE messages from an exchange agents.
    ...
    # Handles QUERY_SPREAD messages from an exchange agents.
    ...
    # Handles QUERY_ORDER_STREAM messages from an exchange agents.
    def queryOrderStream(self, symbol, orders):
        # It is up to the requesting agents to do something with the data, which is a list of dictionaries keyed
    ...
    def query_transacted_volume(self, symbol, transacted_volume):
        """ Handles the QUERY_TRANSACTED_VOLUME messages from the exchange agents"""
    ...
    # perspective, not the agents perspective.  (The agents would be selling into the
    # bid liquidity, etc.)
