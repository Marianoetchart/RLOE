...
class MarketReplayAgent(TradingAgent):
    ...
    def __init__(self, id, name, type, symbol, date, start_time, end_time,
                 starting_cash, orders_file_path, dict_dump_folder_path, dict_dump_path,
        ...
        self.historical_orders = L3OrdersProcessor(self.symbol,
                                                   ...
                                                   orders_file_path, dict_dump_folder_path, dict_dump_path)
...
class L3OrdersProcessor:
    ...
    def __init__(self, symbol, date, start_time, end_time,
                 orders_file_path, dict_dump_folder_path, dict_dump_path):
        self.symbol = symbol
        self.date = date
        ...
        self.dict_dump_folder_path = dict_dump_folder_path
        self.dict_dump_path = dict_dump_path
    ...
    def processOrders(self):
        ...
        if self.dict_dump_path and os.path.isfile(self.dict_dump_path):
            #print(f'\nProcessed file exists for {self.symbol} and {self.date.date()}: {self.dict_dump_path}')
            return read_processed_orders_file(self.dict_dump_path)
        else:
            ...
            dump_file_name = self.dict_dump_folder_path + f'{self.symbol}_{self.date.date()}.pkl'

            with open(dump_file_name, 'wb') as handle:
                pickle.dump(orders_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
                print(f'processed file created as {dump_file_name}')
            return orders_dict
