...
class OrderBook:
    ...
    def __init__(self, owner, symbol):
        self.owner = owner
        self.symbol = symbol
        self.bids = [] # List of bid prices (index zero is best bid), each with a list of LimitOrders.
        self.asks = [] # List of ask prices (index zero is best ask), each with a list of LimitOrders.
        ...
        # Create an order history for the exchange to report to certain agents types.
        self.history = [{}]
    ...
    def handleLimitOrder(self, order):
        ...
        while matching:
            ...
            if matched_order:
                ...
                # No matching order was found, so the new order enters the order book.  Notify the agents.
                self.enterOrder(deepcopy(order))

                log_print("ACCEPTED: new order {}", order)
                log_print("SENT: notifications of order acceptance to agents {} for order {}",
        ...
        if not matching:
            ...
            if executed:
                ...
                #self.history = self.history[:self.owner.stream_history + 1] # TODO: check this thoroughly!!
    ...
    def cancelOrder(self, order):
        ...
        # no message back to the agents.  This should possibly change to some kind of failed
        # cancellation message.  (?)  Otherwise, the agents receives ORDER_CANCELLED with the
            ...
            if self.isEqualPrice(order, o[0]):
                ...
                for ci, co in enumerate(book[i]):
                    if order.order_id == co.order_id:
                        ...
                        log_print("SENT: notifications of order cancellation to agents {} for order {}",
                ...
                for mi, mo in enumerate(book[i]):
                    if order.order_id == mo.order_id:
                        ...
                        for idx, orders in enumerate(self.history):
                            ...
                            log_print("SENT: notifications of order modification to agents {} for order {}",
    ...
    def book_log_to_df(self):
        """ Returns a pandas DataFrame constructed from the order book log, to be consumed by
            agents.ExchangeAgent.logOrderbookSnapshots.
