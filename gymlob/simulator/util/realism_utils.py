...
sys.path.append(p)
from gymlob.simulator.util.convert_order_book import process_orderbook, is_wide_book
from gymlob.simulator.util.convert_order_stream import convert_stream_to_format
import itertools
from bisect import bisect
from matplotlib.cm import get_cmap
import os
import warnings
from gymlob.simulator.util.util import get_value_from_timestamp
...
def make_orderbook_for_analysis(stream_path, orderbook_path, num_levels=5, ignore_cancellations=True,
    ...
    stream_df = pd.DataFrame(pd.read_pickle(stream_path))
    orderbook_df = pd.DataFrame(pd.read_pickle(orderbook_path))
...
def get_daily_spread(orderbook_df, start_time='09:30:00', end_time='16:00:00'):
    """ Get mean spread for the day's trading.
        ...
        :param orderbook_df: preprocessed orderbook (see __name__.make_orderbook_for_analysis) for data without execution agents.
...
def get_execution_agent_vwap(experiment_name, agent_name, date, seed, pov, log_dir):
    """ Function computes the VWAP for an execution agents's orders, when ran from the `execution_iabs_plots` config.

        :param experiment_name: name for experiment
        :param agent_name: name of agents, e.g. POV_EXECUTION_AGENT
        :param date: date of experiment in format YYYY-MM-DD
        :param seed: seed used to run experiment
        :param pov: Participation of volume for agents
...
def compute_impact_statistics(orderbook_df, orderbook_with_execution_df, start_time, end_time, date_str, pov, seed,
                              experiment_name,
    ...
    """ Computes dictionary of run statistics for comparison.
        ...
        :param orderbook_df: preprocessed orderbook (see __name__.make_orderbook_for_analysis) for data without execution agents.
        :param orderbook_with_execution_df: preprocessed orderbook (see __name__.make_orderbook_for_analysis) for data with execution agents.
    ...
    start_mid_price_orig, end_mid_price_orig, end_mid_price_execution = get_relevant_prices(orderbook_df,
